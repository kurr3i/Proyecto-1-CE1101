...
def dibujar_puntos_mapa_recursivo(canvas, frame_padre, tierras, coords, progreso, callback, i):
    ...
    es_nivel_actual = (i == progreso)
    fue_completado = (i < progreso)

    if fue_completado:
        texto_btn = "✅"
        color_btn = "#95a5a6" # Gris (Ya no se puede entrar)
        estado_btn = "disabled"
    elif es_nivel_actual:
        texto_btn = "🪐"
        color_btn = "#5dade2" # Azul (Nivel activo)
        estado_btn = "normal"
    else:
        texto_btn = "🔒"
        color_btn = "#2c3e50" # Oscuro (Bloqueado)
        estado_btn = "disabled"
